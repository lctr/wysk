...
pub use deque::Deque;
pub use link::Link;
pub use maps::{HashMap, Map, MapIntoIter, MapIter, MapIterMut};

pub use sets::{HashSet, Set, SetIntoIter, SetIter};
...
impl Counter {
    ...
    /// Shortcut to avoid casting to `u32`; identical to `increment`,
    /// only the return value is a `u32` so beware of potentially high
    /// counts.
    pub fn incr_u32(&mut self) -> u32 {
